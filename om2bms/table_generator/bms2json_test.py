...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parse .bms and match score.json by MD5, then verify SHA256.")
    ...
    if not verify_sha256(matched_score, bms_sha256):
        print("SHA256 verification failed.", file=sys.stderr)
        print(f"BMS path       : {bms_path}", file=sys.stderr)
        print(f"BMS md5        : {bms_md5}", file=sys.stderr)
        print(f"BMS sha256     : {bms_sha256}", file=sys.stderr)
        print(f"JSON md5       : {matched_score.md5}", file=sys.stderr)
        print(f"JSON sha256    : {matched_score.sha256}", file=sys.stderr)
        print(f"JSON title     : {matched_score.title}", file=sys.stderr)
        sys.exit(3)
...
def verify_sha256(entry: ScoreEntry, bms_sha256: str) -> bool:
    json_sha256 = entry.sha256.strip().lower()
    target_sha256 = bms_sha256.strip().lower()
