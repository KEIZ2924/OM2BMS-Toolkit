...
from om2bms.converter.data_structures import HitSound
    ...
    def parse(self, input_file, osumania_beatmap):
        ...
        def header_hitobjects(line, beatmap):
            ...
            if hit_object_type == "1" or hit_object_type == "5":  # single note
                ...
                raise OsuParseException(
                    "HitObject Error: type " + hit_object_type + " not found in " + line)
            ...
            if beatmap.key_count == 7:
                ...
                hit_object.mania_column = (
                    int(line_separated[0]) // (512 // beatmap.key_count))
            ...
            if type_ln:
                filename = "" if len(
                    hit_object_arg) < 5 else hit_object_arg[5].strip()
                sample_set = int(hit_object_arg[1])
            else:
                filename = "" if len(
                    hit_object_arg) < 4 else hit_object_arg[4].strip()
            ...
            if filename == "" and hitsound_int == 0:
                ...
                custom_index = int(hit_object_arg[3]) if type_ln else int(
                    hit_object_arg[2])
                hs_id = (hitsound_int, sample_set, custom_index, filename)
                if hs_id not in beatmap.hitsounds:
                    hitsound = HitSound(hitsound_int, timing_point, sample_set,
                                        custom_index, filename, OsuBeatmapReader._sample_index)
        ...
        def header_timingpoints(line, beatmap):
            ...
            """
            if not hasattr(beatmap, "bpm_duration"):
                beatmap.bpm_duration = {}
                beatmap._last_bpm = None
                beatmap._last_time = None
            ...
            def get_ms_per_beat(ms, _beatmap):
                ...
                if ms >= 0:
                    ...
                    raise OsuParseException(
                        "Non inherited BPM not found. Timing points are broken")
            ...
            if tp.inherited:
                tp.ms_per_beat = get_ms_per_beat(
                    float(line_separated[1]), beatmap)
                ...
                if beatmap._last_bpm is not None and beatmap._last_time is not None:
                    duration = max(tp.time - beatmap._last_time, 0)
                    beatmap.bpm_duration[beatmap._last_bpm] = beatmap.bpm_duration.get(
                        beatmap._last_bpm, 0) + duration

                beatmap._last_bpm = bpm
                beatmap._last_time = tp.time
        ...
        def keep_main_bpm_only(beatmap):
            """
            Preserves only the longest-lasting BPM as the primary chart BPM.
            """
            if not beatmap.noninherited_tp or not beatmap.bpm_duration:
                return
            ...
            beatmap.bpm_duration = {main_bpm: beatmap.bpm_duration[main_bpm]}
        ...
        def header_events(line, beatmap):
            ...
            if line_separated[0] == "Sample":
                ...
                if filename not in beatmap.filename_to_sample.keys():
                    ...
                    sample = OsuBGSoundEvent(
                        time, filename, OsuBeatmapReader._sample_index)
                    OsuBeatmapReader._sample_index += 1
        ...
        def header_difficulty(line, beatmap):
            ...
            if line_property[0] == "CircleSize":
                ...
                OsuParseException("Attribute Error: " +
                                  line_property[0] + " not found")
        ...
        def header_general(line, beatmap):
            ...
            if line_property[0] == "AudioFilename":
                ...
                audio = HitSound(0, None, 0, 1, audio_filename,
                                 OsuBeatmapReader._sample_index)
                OsuBeatmapReader._sample_index += 1
                ...
                if not int(line_property[1]) == 3:
                    raise OsuGameTypeException(
                        "Beatmap is not an o!m beatmap!")
        ...
        def header_metadata(line, beatmap):
            ...
            if line_property[0] == "Title":
                beatmap.title = line_property[1].strip().replace(
                    "/", "").replace("\\", "")
            elif line_property[0] == "TitleUnicode":
                try:
                    beatmap.title_unicode = line_property[1].strip().encode(
                        "shiftjis")
                ...
                try:
                    beatmap.artist_unicode = line_property[1].strip().encode(
                        "shiftjis")
        ...
        if osumania_beatmap._last_bpm is not None and osumania_beatmap._last_time is not None:
            if len(osumania_beatmap.hit_objects) > 0:
                end_time = max(
                    hit_object.time for hit_object in osumania_beatmap.hit_objects)
            else:
                end_time = osumania_beatmap._last_time

            duration = max(end_time - osumania_beatmap._last_time, 0)
            osumania_beatmap.bpm_duration[osumania_beatmap._last_bpm] = \
                osumania_beatmap.bpm_duration.get(
                    osumania_beatmap._last_bpm, 0) + duration

        keep_main_bpm_only(osumania_beatmap)
